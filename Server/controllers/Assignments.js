...
module.exports.sendChatMessage = function sendChatMessage(req, res, next) {
  var taskId = req.params.taskId;
  var message = req.body.message;
  var sender = req.user;
  console.log("hello here");
  if(message == undefined){
    utils.writeJson(res, {errors: [{ 'param': 'Server', 'msg': 'Missing message query parameter'}],}, 400);
  }
  Assignments.sendChatMessage(taskId,sender,message)
  .then(function(response) {
    utils.writeJson(res, response, 204);
  })
  .catch(function(response){
    if(response == 403){
      utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The task is not active for the user' }], }, 403);
    }
    else if (response == 404){
      utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The task does not exist.' }], }, 404);
    } 
