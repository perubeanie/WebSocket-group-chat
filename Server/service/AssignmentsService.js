...
/**
 * Send a message in the chat of the active task
 *
 * Input: 
 * - taskId: ID of the active task
 * - sender: id of the user who wants to send a message in the chat
 * - message: content of the message
 * Output:
 * - no response expected for this operation
 * 
 **/
 exports.sendChatMessage = function sendChatMessage(taskId, sender, message) {
    return new Promise((resolve, reject) => {
        ...
        db.all(sql1, [taskId], (err, rows) => {
            if (err)
                ...
                const sql2 = 'SELECT a.user FROM assignments as a WHERE a.task = ? AND a.user = ? AND a.active = 1';
                db.all(sql2, [taskId, sender], (err, rows) => {
                    if (err)
                        reject(err);
                    else if (rows.length === 0)
                        reject(403);
                    else {
                        const sql3 = 'INSERT INTO chat(task, user, message) VALUES(?,?,?)';
                        db.run(sql3, [taskId, sender, message], function(err) {
                            if (err) {
                                reject(err);
                            } else {
                                resolve(null);
