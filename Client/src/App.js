...
const Main = () => {
  ...
  //contains the content of the chat of the active task
  const [chatContent, setChatContent] = useState('');

  //indicates whether a user is typing a message in the chat or not
  const [typing, setTyping] = useState('');

  // active task of the user
    ...
    if (loggedIn) {
      for (var i = 0; i < onlineList.length; i++) {
        if (user && user.id === onlineList[i].userId && onlineList[i].taskId) {
          setActiveTask({ id: onlineList[i].taskId, taskName: onlineList[i].taskName });
        }
      }
    }
    else
      setActiveTask('');
  }, [loggedIn, onlineList, user]);
    ...
    <Container fluid>
      ...
      <Switch>
        ...
        <Route path={["/list/:filter"]}>
          {loggedIn ?
            <Row className="vh-100 below-nav">
              <TaskMgr taskList={taskList} filter={activeFilter} onDelete={deleteTask} onEdit={handleEdit} onComplete={completeTask} onCheck={selectTask} onSelect={handleSelectFilter}
                refreshTasks={refreshTasks} onlineList={onlineList} ws={ws} chatContent={chatContent} typing={typing} activeTask={activeTask}></TaskMgr>
...
const TaskMgr = (props) => {
  ...
  const { taskList, filter, onDelete, onEdit, onComplete, onCheck, onSelect, refreshTasks,
    onlineList, ws, chatContent, typing, activeTask } = props;
  ...
  return (
    <>
      ...
      <Col className="col-8">
        ...
        <ContentList
          tasks={taskList} onDelete={onDelete} onEdit={onEdit} onCheck={onCheck} onComplete={onComplete}
          filter={activeFilter} getTasks={refreshTasks} ws={ws} chatContent={chatContent} typing={typing}
