...
const ContentList = (props) => {
  const { tasks, onDelete, onEdit, onCheck, onComplete, filter, getTasks, onSendMessage } = props;
  ...
  return (
    <>
      ...
      />
      {props.filter === 'assigned' ? <ActiveTaskChat task={tasks.filter(t => t.active).length > 0 ? tasks.filter(t => t.active)[0] : undefined} onSendMessage={onSendMessage}/> : <></> }
    </>
  )
}
